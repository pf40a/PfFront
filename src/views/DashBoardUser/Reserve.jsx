  ...
  return (
    <div className="relative">
      ...
      <ul role="list" className="divide-y divide-gray-100">
        {state.map((person) => (
          <li key={person.email} className="w-full flex flex-col md:flex-row justify-between gap-2 sm:gap-4 py-4 md:py-6 lg:py-8 relative">
            <div className="flex flex-col md:flex-row items-center min-w-0 gap-2 md:gap-4">
              <img className="h-12 w-12 md:h-16 md:w-16 flex-none rounded-full bg-gray-50" src={person.image} alt="" />
              <div className="min-w-0 flex-auto">
                <p className="text-sm font-semibold md:text-base lg:text-lg leading-6 text-gray-900">{`${person.nombre}  ${person.apellido}`}</p>
                <p className="mt-1 text-xs md:text-sm lg:text-base leading-5 text-gray-500">{person.email}</p>
              </div>
            </div>
